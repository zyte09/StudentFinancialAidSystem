...
public class StudentFinancialAidSystemAlpha {
    public static void main(String[] args){
        ...
        Map<String, List<String>> selectedScholarshipMap = new HashMap<>();
        Map<Integer, String> otherscholarships = new HashMap<>();
        String selectedOtherscholarship = "";
        while (true) {
            ...
            if (!option.matches("[1-6]")) {
                ...
                if (i == -1) {
                        ...
                        if (scholarships.containsKey(formatMajor)) {
                            ...
                            while (!validChoice) {
                                ...
                                if (choice >= 1 && choice <= availableScholarships.size()) {
                                    ...
                                    List<String> studentSelectedScholarships = selectedScholarshipMap.getOrDefault(studentCode, new ArrayList<>());
                                    studentSelectedScholarships.add(selectedScholarship);
                                    selectedScholarshipMap.put(studentCode, studentSelectedScholarships);
                    ...
                    // Scholar
                    System.out.println("\nDO YOU WANT TO APPLY FOR OTHER SCHOLARSHIP PROGRAM?");
                    System.out.println("PRESS 'Y' TO YES, PRESS 'N' TO NO");
                    String applyOtherScholarship = input.nextLine().toUpperCase();
                    
                    if (applyOtherScholarship.equals("Y")) {
                        // Other scholarship progs
                        System.out.println("");
                        otherscholarships.put(1, "SCHOLARSHIP: Student Assistantship Program (SAP) \n>> 100% discount on tuition fees");
                        otherscholarships.put(2, "SCHOLARSHIP: Batangas Province Scholarship Program (BAPROSA) \n>> 5,000 to 10,000 financial assistance");
                        otherscholarships.put(3, "SCHOLARSHIP: CHED Tulong Dunong Program (TDP) \n>> 7,500 per semester");
                        otherscholarships.put(4, "SCHOLARSHIP: Lipa City Scholarship Program (EBA) \n>> 100% discount on tuition fees*");
                        otherscholarships.put(5, "SCHOLARSHIP: CHED Scholarship Program (CSP) \n>> Full Merit = 30,000 per semester  \n>> Half Merit = 15,000 per semester");
                        otherscholarships.put(6, "SCHOLARSHIP: Sto. Tomas Scholarship Program (EPS) \n>> 100% discount on tuition fees, registration, and miscellaneous fees*");
                        otherscholarships.put(7, "SCHOLARSHIP: Dr. Eustacio and Trinidad Leviste-Endaya Scholarship Program (Malvar) \n>> 100% discount on tuition fees, registration, and miscellaneous fees*");
                        ...
                        if (otherscholarships.containsKey(otherscholarshipChoice)) {
                            selectedOtherscholarship = otherscholarships.get(otherscholarshipChoice); // Assign the selected scholarship here
                            System.out.println("YOU CHOSE: " + selectedOtherscholarship);
                            scholarships.put(formatMajor, Collections.singletonList("Your Scholarship Information Here"));
                        } else {
                            System.out.println("INVALID CHOICE! PLEASE SELECT A VALID CHOICE.");
                        }
                    } else {
                        if (!selectedScholarshipMap.containsKey(studentCode)) {
                        System.out.println("YOU CHOSE NOT TO APPLY FOR THE OTHER SCHOLARSHIP PROGRAMS");
                        }
                    }
                } 
            } else {
                ...
                if (i == -1) {
                    ...
                    List<String> selectedScholarships = selectedScholarshipMap.get(studentCode);

                    selectedScholarshipMap.put(studentCode, Collections.singletonList(selectedOtherscholarship));

                    if (selectedScholarships != null && !selectedScholarships.isEmpty()) {
                        System.out.print("SELECTED SCHOLARSHIP:");
                        for (String scholarship : selectedScholarships) {
